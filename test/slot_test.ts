...
Deno.test('slots', async (test) => {
  ...
  await test.step('named slot (1)', async () => {
    const html = `<named-slot><ssr-fragment slot="start">Start!</fragment></named-slot>`;
    const output = await hypermore.render(html);
    assertEquals(output, `<main>Start! Center! </main>`);
  });
  await test.step('named slot (2)', async () => {
    const html = `<named-slot><ssr-fragment slot="start">Start!</fragment><ssr-fragment slot="end">End!</fragment></named-slot>`;
    const output = await hypermore.render(html);
    assertEquals(output, `<main>Start! Center! End!</main>`);
  });
  await test.step('named slot + unused', async () => {
    const html = `<named-slot><ssr-fragment slot="end">End!</fragment></named-slot>`;
    const output = await hypermore.render(html);
    assertEquals(output, `<main>Unused! Center! End!</main>`);
  });
  await test.step('named slot + default', async () => {
    const html = `<named-slot><ssr-fragment slot="start">Start!</fragment> Middle! <ssr-fragment slot="end">End!</fragment></named-slot>`;
    const output = await hypermore.render(html);
    assertEquals(output, `<main>Start! Middle! End!</main>`);
  });
  await test.step('named slot + props', async () => {
    const html = `<named-slot start="Start!" end="End!"><ssr-fragment slot="start">{{start}}</fragment><ssr-fragment slot="end">{{end}}</fragment></named-slot>`;
    const output = await hypermore.render(html);
    assertEquals(output, `<main>Start! Center! End!</main>`);
  });
});
